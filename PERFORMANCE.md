...
| Yap (1997) | Exact geometric computation benefits from separating combinatorial decisions from numerical realization, filtering easy cases, and delaying exact expansion/division. | Already represented by `Real`-backed structural facts, zero/support masks, immediate matrix dispatch, shared-scale views, homogeneous/projective carriers, and delayed affine division. These facts let downstream predicates choose a small certified route without giving up exact fallback. |

## Immediate matrix API gate

The public matrix API no longer exposes cache, right-divisor, transformed-value,
or materialization lifecycles. `Matrix3` and `Matrix4` now own immediate inverse,
division, scalar transform, and batch-transform dispatch. Batch calls may retain
facts inside one invocation, but that implementation detail does not escape as a
caller-managed object.

The serialized pre-change gate measured the public dense Matrix3 batch transform
at 3.2248 us, the public Matrix4 direction batch at 2.3223 us, and direct checked
Matrix4 division at 7.6979 us. The matching post-change medians were 1.8641 us
(42.2% faster), 2.3565 us (no statistically detectable change), and 7.8231 us
(within Criterion's 2% noise threshold), respectively.
...
appropriate for explicit `structural_facts` queries and internal batch dispatch,
but were dead work for a one-shot transform.
...
owned/borrowed, and batch equivalence tests protect the same exact dispatch
decisions. No approximation or new public lifecycle is involved.
...
`src/matrix/core.rs` thirteen lines smaller; public structural facts and scalar
arithmetic internals are unchanged.
...
affine, diagonal, batch, and transform paths remain separately observable.
